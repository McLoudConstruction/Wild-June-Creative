import type { Block } from '@/lib/site/blocks';
import { Hero } from './Hero';
import { FeaturedPhoto } from './FeaturedPhoto';
import { Sessions, type SessionPackage } from './Sessions';
import { PortfolioGrid } from './PortfolioGrid';
import { About } from './About';

// Renders an ordered list of blocks in place. Used by the public page
// (app/page.tsx) and, unchanged, by the admin editor's live preview —
// so what Isabelle sees while editing is exactly what ships.
export function BlockRenderer({
  blocks,
  packages,
}: {
  blocks: Block[];
  packages: SessionPackage[];
}) {
  return (
    <>
      {blocks.map((block) => {
        switch (block.type) {
          case 'hero':
            return <Hero key={block.id} {...block.props} />;
          case 'featured_photo':
            return <FeaturedPhoto key={block.id} {...block.props} />;
          case 'sessions':
            return <Sessions key={block.id} {...block.props} packages={packages} />;
          case 'portfolio_grid':
            return <PortfolioGrid key={block.id} {...block.props} />;
          case 'about':
            return <About key={block.id} {...block.props} />;
          default:
            return null;
        }
      })}
    </>
  );
}
